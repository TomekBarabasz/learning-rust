...
fn split(input:&str) ->Vec<&str> {
    ...
    let mut is_char = input.chars().next().unwrap().is_alphabetic();
    let mut start = 0;
...
fn align(input : Vec<&str>) -> Vec<String> {
    ...
    input
        .iter()
        .map(|s| {
            let to_pad = max_len - s.len();
            let prep = std::iter::repeat('_').take(to_pad).collect::<String>();
            prep+s
        })
        .collect()
}
...
pub fn solve(input: &str) -> Option<HashMap<char, u8>> {
    ...
    let lines = align(lines);
    dbg!(lines);
    let ret : HashMap<char,u8> = HashMap::new();
    Some(ret)
}

fn first_permutation(m : u8) -> Vec<u8> {
    [0..=m]
        .map(|i| i as u8)
        .collect()
}
fn next_permutation(state : Vec<u8>) -> Option<Vec<u8>> {
